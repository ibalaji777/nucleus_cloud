<template>
  <div class="bgGradient">
    <div class="menubar">
      <div
        @click="defaultMenuBar = item.name"
        :class="{ selectedNavbar: defaultMenuBar == item.name }"
      ...
      <div>
        ...
        <div v-if="defaultMenuBar == 'dashboard'">
          <div style="display: flex; flex-wrap: wrap">
            ...
            <div
              @click="$router.push({ name: 'main_users' })"
...
Users
      ...
      >
        Group
            ...
            <div
              @click="
                $store.commit('dialog', { key: 'addShiftDialog', value: true })
              "
              class="nucleus_widget gradient5"
            >
              Shifts
...
Emp Role
          ...
          <h4
            v-if="branchWiseMachine.length == 0"
            style="
              text-align: center;
              height: 450px;
      ...
      <div style="padding: 10px; background: white">
        <div class="side_nav_widget">
          <div class="side_nav_widget_split">
            <div style="background: #f5fafe; flex: 1">
              {{ branchWiseMachine.length }}
            </div>
        ...
        <div class="side_nav_widget">
          <div class="side_nav_widget_split">
            ...
            <div style="background: white; flex: 2">EMPLOYEE</div>
          </div>
        </div>
      </div>
    </div>
  ...
  computed: {
    branchWiseProducts() {
      ...
      return _.filter(
        ...
        (machine) => machine.branch == $vm.$store.state.setup.selected_branch
      );
    },
    branchWiseShifts() {
      var $vm = this;
      return _.filter(
        $vm.$store.state.db.shift,
        (machine) => machine.branch == $vm.$store.state.setup.selected_branch
      );
    },
    ...
    employeesList() {
      var $vm = this;
      return _.filter(
        $vm.$store.state.db.employees,
        (emp) => emp.branch == $vm.$store.state.setup.selected_branch
      );
    },
